...
public class ArrownextclickedProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		if (entity == null)
			return;
		if (entity instanceof Player _plr0 && _plr0.containerMenu instanceof ConstructorcorerecipeselectMenu) {
			if (entity instanceof ServerPlayer _ent) {
				...
				NetworkHooks.openScreen((ServerPlayer) _ent, new MenuProvider() {
					...
					public Component getDisplayName() {
						return Component.literal("Constructioncorerecipeselect2");
					}

					@Override
					public AbstractContainerMenu createMenu(int id, Inventory inventory, Player player) {
						return new Constructioncorerecipeselect2Menu(id, inventory, new FriendlyByteBuf(Unpooled.buffer()).writeBlockPos(_bpos));
					}
				}, _bpos);
			}
		}
